...
RAG（检索增强生成）系统为您的文档提供 AI 驱动的聊天功能。

### 工作流程

```
用户查询 → 查询改写 → 混合搜索 → 上下文构建 → LLM 回复
              ↓            ↓             ↓
          LLM 增强     向量 + BM25    排序与过滤
```

### 核心特性

- **混合搜索**: 结合向量相似度 (pgvector) 与 BM25 关键词搜索
- **动态 Top-K**: 根据查询复杂度动态调整检索数量
- **MMR 重排序**: 最大边际相关性算法提升结果多样性
- **Markdown 感知分块**: 保留文档结构，支持标题感知
- **查询改写**: 使用 LLM 结合对话历史扩展短查询/指代词
- **多文档合成**: 检测跨文档问题并综合信息

### 文档处理流程

```
上传 → 文本提取 → 分块 → 向量嵌入 → 向量存储
    (PDF/DOCX/TXT)  (800-1500 字符) (text-embedding-v4) (pgvector)
```

### 搜索流程

```
查询 → 向量化 → 向量搜索 → 关键词搜索 → RRF 融合 → MMR → 上下文
              ↓                              ↓
          top-K 结果                倒数排名融合
```
